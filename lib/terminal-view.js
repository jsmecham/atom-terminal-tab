...
export default class TerminalView {
  ...
  applyThemeStyles() {
    var styles = '';
    if(typeof atom.config.settings.editor.fontSize !== 'undefined') styles += 'font-size: ' + atom.config.settings.editor.fontSize + 'px; ';
    if(typeof atom.config.settings.editor.fontFamily !== 'undefined') styles += 'font-family: ' + atom.config.settings.editor.fontFamily + '; ';
    if(typeof atom.config.settings.editor.lineHeight !== 'undefined') styles += 'line-height: ' + atom.config.settings.editor.lineHeight + '; ';
    const termEls = document.getElementsByClassName('terminal xterm');
    window.setTimeout(() => {
      for(let i in termEls) {
        const termEl = termEls[i];
        if(typeof termEl === 'object') termEl.setAttribute('style', styles);
      }
    });
  }
